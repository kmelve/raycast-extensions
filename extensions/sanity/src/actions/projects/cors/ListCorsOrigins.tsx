...
import { CorsOrigin } from "../../../types/CorsOrigin";
...
export function ListCorsOrigins(props: { project: SanityProject }) {
  ...
  const [origins, setOrigins] = useState<CorsOrigin[] | null>(null);

  const deleteOriginHandler = async (id: string) => {
    await client.request({ url: `/cors/${id}`, method: "DELETE" }).catch((err) => console.log(err));
